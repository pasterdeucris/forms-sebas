"""
Tests unitarios para el formulario de Preescolar Integrales de Colsubsidio - Form4
"""

import pytest
from unittest.mock import Mock, MagicMock, patch
from form4 import ColsubsidioFormFiller, SECCIONES_CONFIG


class TestColsubsidioFormFillerForm4:
    """Tests para la clase ColsubsidioFormFiller del formulario 4"""

    @pytest.fixture
    def form_filler(self):
        """Fixture que crea una instancia del form filler con driver mockeado"""
        with patch('form4.webdriver.Chrome'):
    ...
    def test_inicializacion(self, form_filler):
        ...
        assert form_filler.url == "https://colsubsidio.az1.qualtrics.com/jfe/form/SV_39rtVbeLsFoU9Bc"
    ...
    def test_esperar_carga(self, form_filler):
        ...
        with patch('form4.time.sleep') as mock_sleep:
    ...
    def test_llenar_pagina_1(self, form_filler):
        ...
        form_filler.driver.find_element = Mock(side_effect=[
            mock_proyecto, mock_radio, mock_radio
        ])

        with patch('form4.WebDriverWait') as mock_wait:
    ...
    def test_llenar_seccion_acompanamiento_familia(self, form_filler):
        """Test que verifica el llenado de la sección acompañamiento familia"""
        form_filler.seleccionar_escala_matriz = Mock()

        valores = [10, 9, 8, 7, 6]
        form_filler.llenar_seccion('acompanamiento_familia', valores)

        # 'acompanamiento_familia' tiene 5 choice_ids
        assert form_filler.seleccionar_escala_matriz.call_count == 5

    def test_llenar_seccion_coordinador_pedagogico(self, form_filler):
        """Test que verifica el llenado de la sección coordinador pedagógico"""
        form_filler.seleccionar_escala_matriz = Mock()

        valores = [10, 9, 8, 7]
        form_filler.llenar_seccion('coordinador_pedagogico', valores)

        # 'coordinador_pedagogico' tiene 4 choice_ids
    ...
    def test_responder_pregunta_si_no_spe_desa(self, form_filler):
        """Test que verifica responder a pregunta especialista desarrollo"""
        mock_label = Mock()
        form_filler.driver.find_element = Mock(return_value=mock_label)

        form_filler.responder_pregunta_si_no_spe_desa("Si")
    ...
    def test_llenar_sugerencias(self, form_filler):
        """Test que verifica el llenado del campo de sugerencias"""
        mock_field = Mock()
        form_filler.driver.find_element = Mock(return_value=mock_field)

        form_filler.llenar_sugerencias("Sugerencias de prueba")

        mock_field.clear.assert_called_once()
        mock_field.send_keys.assert_called_with("Sugerencias de prueba")
